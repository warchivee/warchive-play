<script lang='ts'>
    ...
    import PrizeBorderVer from '$lib/images/mockexam/systems/prize_border_ver.png';
     ...
     let scores = urlData.searchParams.get('s')?.split(',')?.map(s => Number(s) ?? 0) ?? [0, 0, 0, 0, 0];
...
<section>
     ...
     <div class='result-popup'>
        ...
        <div class='result-popup__footer'>
            <button on:click={() => {goto(`${base}/master-mock-exam/review`); }}>해설 확인하기</button>
            <button on:click={() => {goto(`${base}/master-mock-exam`); }}>다시 응시하기</button>
        </div>
    </div>
    ...
    <div class='prize' style="--prize_border_hor: url({PrizeBorderHor}); --prize_border_ver: url({PrizeBorderVer});">
